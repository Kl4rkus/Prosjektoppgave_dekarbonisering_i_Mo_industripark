...
co2_bio_pre_def(t)..
    CO2_bio_pre(t) =e=
          sum(s, sum(fuel_bio,
                        EF_bio(fuel_bio)
                      * ( uSAF(s,t,fuel_bio)
                        + uSAF_SiMn(s,t,fuel_bio) )))
        + sum(fuel_lt$(fuel_bio(fuel_lt)),
                EF_bio(fuel_lt) * uSint_LT(t,fuel_lt))
        + sum(fuel_ht$(fuel_bio(fuel_ht)),
                EF_bio(fuel_ht) * uSint_HT(t,fuel_ht));


* ETS-accounted emissions:
*   physical net emissions (CO2_total_net)
*   minus all biogenic CO2 (treated as climate-neutral),
* which automatically allows negative emissions when CCS captures more
...
TCF_FeMn(s,t) = 2700;
*TCF_SiMn(s,t) = 4500;
TCF_SiMn_Si(s,t)   = 2500;
TCF_SiMn_SiO2(s,t) =       4500;
TCF_sint(t)   = 1000;
