...
*-----------------------------
SETS
    t              "time periods"                    / t1*tN /    * TODO: replace with your periods
    saf            "SAF index"                       / s1, s2 /
    lhFuel         "low-heat fuels (LT)"             / oil, biooil, woodchips /
    hhFuel         "high-heat fuels (HT)"            / coke, COgas, hydrogen, natgas, biogas, biochar /
    fSiMn          "fuel techs for SiMn LT/HT terms" / f1*fK /    * TODO: detail if you want distinct techs for SiMn LT/HT
;
...
* Demands
PARAMETERS
    DFeMn(t)               "demand FeMn at time t"
    DSiMn(t)               "demand SiMn at time t"
;

* Yields and coefficients
SCALARS
    Y_ore_sint             "Y_{ore->sinter}^{sint}"
    Y_sin_FeMn             "Y_{sinter->FeMn}^{SAF}"
    Y_sin_slag             "Y_{sinter->slag}^{SAF}"
    eta_sint               "electricity per t sinter"
;

PARAMETERS
    eta1_SAF(t)            "kWh/t FeMn in SAF1"
    eta2_SAF(t)            "kWh/t FeMn in SAF2"
    nu1_SAF(t)             "kWh/t SiMn in SAF1"
    nu2_SAF(t)             "kWh/t SiMn in SAF2"
    Y1_offgas(t)           "off-gas t per t FeMn in SAF1"
    Y2_offgas(t)           "off-gas t per t FeMn in SAF2"
    Z1_offgas_SiMn(t)      "off-gas per t SiMn SAF1"
    Z2_offgas_SiMn(t)      "off-gas per t SiMn SAF2"
    D_offgas(t)            "off-gas demand"
;

* Emission factors
SCALARS
    EF_coke_sint           "tCO2 per t coke (sinter)"
    EF_flare               "tCO2 per t off-gas flared"
;

* Heat proportion coefficients (K) and minimum HT factors (A)
PARAMETERS
    KLT_sint(t)            "K_{t,LT}^{sint}"
    KHT_sint(t)            "K_{t,HT}^{sint}"
    AHT_sint               "A_{HT}^{sint}"

    KLT_SAF1(t)            "K_{1,t,LT}^{SAF}"
    KHT_SAF1(t)            "K_{1,t,HT}^{SAF}"
    AHT_SAF1               "A_{1,HT}^{SAF}"

    KLT_SAF2(t)            "K_{2,t,LT}^{SAF}"
    KHT_SAF2(t)            "K_{2,t,HT}^{SAF}"
    AHT_SAF2               "A_{2,HT}^{SAF}"

    TCF_SiMn1(t)           "total LT+HT per t SiMn in SAF1"
    TCF_SiMn2(t)           "total LT+HT per t SiMn in SAF2"
    AHT_SAF1_SiMn          "A_{1,HT}^{SAF,SiMn}"
    AHT_SAF2_SiMn          "A_{2,HT}^{SAF,SiMn}"
;

* Fuel-to-heat conversion factors phi (kWh heat per unit fuel)
PARAMETERS
    phi_sint_lh(t,lhFuel)  "LT sinter"
    phi_SAF1_lh(t,lhFuel)  "LT SAF1"
    phi_SAF2_lh(t,lhFuel)  "LT SAF2"
